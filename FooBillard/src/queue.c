...
static double cue_shad_w = 0.07;
static double cue_shad_w2 = 0.014;
static int queuetexbind;


double rfunc(double x)
{
  return(cos(x*M_PI));
}
...
int create_queue(double(*rfunc)(double))
{
  ...
  for (j = 0; j < segnr; j++){
    ...
    r1 = 0.5*(QUEUE_D1 - (QUEUE_D1 - QUEUE_D2)*(rfunc((double)j / segnr) - rfunc(0)) / (rfunc(1) - rfunc(0)));
    r2 = 0.5*(QUEUE_D1 - (QUEUE_D1 - QUEUE_D2)*(rfunc((double)(j + 1) / segnr) - rfunc(0)) / (rfunc(1) - rfunc(0)));

    dr1 = 0.5*(QUEUE_D1 - (QUEUE_D1 - QUEUE_D2)*(rfunc((double)j / segnr + 0.001) - rfunc(0)) / (rfunc(1) - rfunc(0)));
    dr1 = (dr1 - r1) / 0.001;
    dr2 = 0.5*(QUEUE_D1 - (QUEUE_D1 - QUEUE_D2)*(rfunc((double)(j + 1) / segnr + 0.001) - rfunc(0)) / (rfunc(1) - rfunc(0)));
    dr2 = (dr2 - r2) / 0.001;

    for (i = 0; i < sidenr + 1; i++){
      ph = 2.0*M_PI / (double)sidenr*i;
      /*           v1.x = QUEUE_D1/2.0*cos(ph);
                 ...
                 v2.z = dl2;
                 n.x = (QUEUE_L-dl1-dl2)*cos(ph);
                 n.y = (QUEUE_L-dl1-dl2)*sin(ph);
                 n.z = -(QUEUE_D1-QUEUE_D2)/2.0;*/
      v1.x = r1*cos(ph);
      v1.y = r1*sin(ph);
  ...
  for (i = 0; i < sidenr + 1; i++){
    ph = 2.0*M_PI - 2.0*M_PI / (double)sidenr*i;
    v1.x = QUEUE_D1 / 2.0*cos(ph);
    v1.y = QUEUE_D1 / 2.0*sin(ph);
    v1.z = QUEUE_L - dl1;
    glTexCoord2f(1.0 - v1.z / QUEUE_L, (v1.x + QUEUE_D1 / 2.0) / QUEUE_D1);
  ...
  GLfloat xoffs, GLfloat yoffs, int spheretexbind, VMvect * lightpos, int lightnr)
{
  ...
  if (!init){
    if (options_queue_tex){
      ...
      gluBuild2DMipmaps(GL_TEXTURE_2D, 1, queuetexw, queuetexh, GL_LUMINANCE,
        GL_UNSIGNED_BYTE, queuetexdata);
      free(queuetexdata);
  ...
  //    glTranslatef( 0.0, 0.0, zoffs );
  glBindTexture(GL_TEXTURE_2D, queuetexbind);
  glCallList(queue_obj);

  /* draw queue-reflections */
  if (options_queue_reflect){
  ...
  /* draw queue-shadow */
  if (options_queue_shadows){
    if (options_simple_queue_shadows){
      glDisable(GL_TEXTURE_GEN_S);
      glDisable(GL_TEXTURE_GEN_T);
      ...
      glTranslatef(pos.x, pos.y, -BALL_D / 2.02);
      glRotatef(-Zrot, 0.0, 0.0, 1.0);
      //            glTranslatef( 0.0, -zoffs, 0.0 );
      glBindTexture(GL_TEXTURE_2D, queueshadowbind);
      glBegin(GL_QUADS);

      glNormal3f(0.0, 0.0, 1.0);
      glTexCoord2f(0.0, 0.0);
      glVertex3f(-cue_shad_w, 0.0, 0.0);
      ...
      glVertex3f(+cue_shad_w, -4.0*cue_shad_w, 0.0);
    ...
    if (!options_simple_queue_shadows){
      ...
      for (i = 0; i < lightnr; i++){
        ...
        /*                p1= vec_xyz(0.2,0.5,-0.028292);
                        p2= vec_xyz(-0.2,-0.5,-0.028292);*/
...
#define shoulder1_h 0.50
    ...
    VMvect shoulder1, shoulder2, shoulder_dir, hand1, hand2, elbow1, elbow2, xdir, ydir;
    double x1, x;
    ...
    //        elbow2 = vec_add(vec_scale(xdir,x2),vec_scale(ydir,sqrt(forearm_l*forearm_l-x2*x2)));
    ...
    //           glVertex3f( elbow2.x,    elbow2.y,    elbow2.z    );
