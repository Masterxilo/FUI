...
void create_caramboltex_binds(void)
{
  ...
  for (i = 0; i < 3; i++){
    glGenTextures(1, &balltexbind[i]);
    sprintf(str, "caramball%02d.png", i);
    if (options_autocreate_balltex){
...
void create_snookertex_binds(void)
{
  ...
  for (i = 0; i < 8; i++){
    glGenTextures(1, &balltexbind[i]);
    sprintf(str, "snookerball%02d.png", i);
    if (options_autocreate_balltex){
...
{
  int i;
  glColor3f((double)(((options_col_ball[pball->nr]) >> 16) & 0xFF) / 255.0,
    (double)(((options_col_ball[pball->nr]) >> 8) & 0xFF) / 255.0,
    (double)(((options_col_ball[pball->nr]) >> 0) & 0xFF) / 255.0);
